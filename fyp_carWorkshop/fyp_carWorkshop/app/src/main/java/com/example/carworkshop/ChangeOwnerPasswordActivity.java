...
public class ChangeOwnerPasswordActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        getSupportActionBar().setTitle("Change Owner Password");
        getSupportActionBar().setDisplayHomeAsUpEnabled(true);
        toolbar.setNavigationOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                startActivity(new Intent(ChangeOwnerPasswordActivity.this,OwnerHomepageActivity.class));
            }
        });
        ...
        changePsw.setOnClickListener(new View.OnClickListener(){
            ...
            public void onClick(View v) {
                ...
                if(txtOldPsw.isEmpty() || txtNewPsw.isEmpty() || txtConfirmPsw.isEmpty()){
                    Toast.makeText(ChangeOwnerPasswordActivity.this,"All fields are required", Toast.LENGTH_SHORT).show();
                }
                else if(txtNewPsw.length() < 6){
                    Toast.makeText(ChangeOwnerPasswordActivity.this,"The new password length should be more then 6 character",Toast.LENGTH_SHORT).show();
                }
                else if (! txtConfirmPsw.equals(txtNewPsw)){
                    Toast.makeText(ChangeOwnerPasswordActivity.this,"Confirm password is no match will new password",Toast.LENGTH_SHORT).show();
                }
                else{
                    ...
                    firebaseUser.updatePassword(txtNewPsw).addOnCompleteListener(new OnCompleteListener<Void>() {
                        ...
                        public void onComplete(@NonNull Task<Void> task) {
                            if (task.isSuccessful()){
                                ...
                                Intent intent = new Intent(ChangeOwnerPasswordActivity.this,MainActivity.class);
                                intent.setFlags(Intent.FLAG_ACTIVITY_CLEAR_TASK | Intent.FLAG_ACTIVITY_NEW_TASK);
                                startActivity(intent);
                            }
                            else{
                                Toast.makeText(ChangeOwnerPasswordActivity.this, task.getException().getMessage(),Toast.LENGTH_SHORT).show();
                            }
                        }
                    });
                }
